...
5. Treat only `succeeded` or `reconciled` as completed.
...
In development, Swagger at `/docs` can emulate upstream resources and outcomes.
Every sandbox result says `simulated: true`; it is not evidence of a real refund,
