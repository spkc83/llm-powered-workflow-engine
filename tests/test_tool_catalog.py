...
def test_consequential_tools_declare_independent_controls() -> None:
    ...
    for entry in consequential:
        ...
        assert entry.production_model_enabled is False


def test_production_model_exposure_fails_closed_for_consequential_tools() -> None:
    ...
    assert "issue_refund" not in selected_names
    assert "issue_store_credit" not in selected_names
    assert "file_eft_dispute" not in selected_names
    assert "issue_provisional_credit" not in selected_names
...
def test_customer_service_agent_applies_production_freeze(monkeypatch) -> None:
    ...
    assert "lookup_order" in selected_names
    assert "issue_refund" not in selected_names
    assert "never claim that a frozen action succeeded" in agent.instruction.lower()
...
def test_fraud_agent_applies_production_freeze(monkeypatch) -> None:
    ...
    assert "flag_account" not in selected_names
    assert "submit_sar" not in selected_names
    assert "never claim that a frozen action succeeded" in agent.instruction.lower()
